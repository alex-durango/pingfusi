...
- `tunnel.json` — a public tunnel url (`harness/tunnel.js` records it only after
  byte-verifying it serves `clone/index.html`, and records which `transport` carried
  it); the `--draft` fallback when no hosted draft exists — reserved for adopted builds
  that genuinely require a live server. Transport order is named cloudflared tunnel →
  ngrok → anonymous quick tunnel; the quick tunnel is a warned last resort because
  Cloudflare rate-limits those (a 429 mid-round burns the round). Provision a named one
  once: `cloudflared tunnel create pingfusi`, `cloudflared tunnel route dns pingfusi
  <host>`, then `~/.cloudflared/pingfusi-tunnel.json` = `{"name","hostname"}`.
